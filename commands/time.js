...
module.exports = {
	...
	execute(message, args) {
        ...
        if (tMinus < (8 * 60 * 60 * 1000)) {
            message.channel.send(`:soup: Coffee time :soup: is today at ${coffeeTime.hour}:00 PST!`)
        } else {
            message.channel.send(`:soup: Coffee time :soup: is tomorrow at ${coffeeTime.hour}:00 PST!`)
        }
	},
}